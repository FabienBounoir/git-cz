...
const getTypeConfig = (config, type) => {
  ...
  const givenType = type === undefined || type === null || type === '' ? '<empty>' : String(type);

  throw new TypeError(`Invalid commit type "${givenType}". Expected one of: ${validTypes.join(', ')}`);
};
